...
\begin{frame}[fragile]{Miscellaneous}
  \begin{itemize}
    ...
    \item \texttt{Kokkos::Profiling::ScopedRegion}
...
Currently, we can't use the same functor in both a top-level parallel construct and hierarchical parallelism. The following will be allowed in the future:
\begin{code}
...
\begin{frame}[fragile]{C++20 Concepts}
\begin{itemize}
\item \texttt{ExecutionPolicy}
\item \texttt{ExecutionSpace}
\item \texttt{MemorySpace}
\item \texttt{Reducer}
\item \texttt{TeamHandle}
\end{itemize}
\end{frame}
...
\item Full support for mathematical functions
...
\begin{frame}
  ...
  \vfill
  \url{https://kokkos.org}\qquad \url{https://github.com/kokkos}
  \end{frame}
